...
def draw_3x3_board(board, x, y):
    ...
    for i in range(0, 3):
        for j in range(0, 3):
            ...
            pygame.draw.rect(SCREEN, BOARDER_COLOR,
            ...
            if board[3 * j + i] == gp.PLAYER1_MARKER:
                pygame.draw.circle(SCREEN, P0_COLOR,
                                   ...
                                   SQUARE_SIDE / 2 * 0.7, 10)

            if board[3 * j + i] == gp.PLAYER0_MARKER:
                pygame.draw.line(SCREEN, P1_COLOR,
                                 (int(x + i * SQUARE_SIDE) + 20, int(y + j * SQUARE_SIDE) + 20),
                                 (int(x + i * SQUARE_SIDE + SQUARE_SIDE) - 20,
                                  int(y + j * SQUARE_SIDE + SQUARE_SIDE) - 20),
                                 10)

                pygame.draw.line(SCREEN, P1_COLOR,
                                 (int(x + i * SQUARE_SIDE + SQUARE_SIDE) - 20, int(y + j * SQUARE_SIDE) + 20),
                                 (int(x + i * SQUARE_SIDE) + 20, int(y + j * SQUARE_SIDE + SQUARE_SIDE) - 20),
                                 10)
...
def overlay_decided(board_wins, x, y):
    for i in range(0, 9):
        marker = board_wins[i]
        if marker == gp.PLAYER1_MARKER:
            # TODO make based on color variables
            color = (220, 17, 30, 100)
        elif marker == gp.PLAYER0_MARKER:
...
def draw_game_board(main_board, main_board_wins, current_local_board, X_OFFSET, Y_OFFSET):
    ...
    draw_board_overlay(current_local_board, X_OFFSET, Y_OFFSET, (134, 136, 138, 80))
